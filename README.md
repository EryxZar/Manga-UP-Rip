...
*(Nota: Los capítulos que requieren ver un anuncio / "Video Reward" actualmente se saltan en la versión GUI. Para esos capítulos, utiliza la versión de consola).*

### 🚀 Instalación y Uso

**Opción A: Usar el archivo ejecutable (Fácil)**
1. Ve a la pestaña **[Releases](https://github.com/TU_USUARIO/TU_REPOSITORIO/releases)** en la derecha de esta página.
...
1. Instala una aplicación de proxy o captura de red en tu teléfono y PC (como **HTTP Toolkit** o **mitmproxy**).
...
6. Abre el programa en tu PC, haz clic en **Configurar UUID**, pégalo y dale a Guardar. ¡Listo!
...
Instead, the program relies on a **UUID** system. You only need to log in ONCE on the official app (using a proxy) and capture your UUID. The program will save it and reuse it for all downloads without needing to log in again.
...
*(Note: Chapters that require watching an ad / "Video Reward" are currently skipped in the GUI version. For those chapters, please use the console version).*

### 🚀 Installation and Usage

**Option A: Using the executable (Easy)**
1. Go to the **[Releases](https://github.com/TU_USUARIO/TU_REPOSITORIO/releases)** tab on the right side of this page.
2. Download the latest `.exe` file.
3. Run it (no Python installation required).

**Option B: From source code**
1. Clone this repository: `git clone https://github.com/TU_USUARIO/TU_REPOSITORIO.git`
...
1. Install a proxy/network capture app on your phone and PC (like **HTTP Toolkit** or **mitmproxy**).
2. Open the real *Manga UP!* app with the proxy active and log in.
3. Look for any request made after logging in (e.g., `.../start?...` or `.../my_page?...`).
4. Inside the URL of that request, you will see a parameter called `&uuid=`.
5. Copy that code (it looks like `XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX`).
6. Open the program on your PC, click **Configure UUID**, paste it, and save. You're good to go!

---
*Disclaimer: This tool is for personal and educational use only. Please support the official creators and platforms.*
